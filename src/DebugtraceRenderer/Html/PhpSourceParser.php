...
namespace KnotLib\ExceptionHandler\DebugtraceRenderer\Html;

use KnotLib\ExceptionHandler\Exception\PhpSourceParserException;
...
//    const OPERATOR_ELEMENTS    = '&& || += -= .= + - * / % = == === < > <= >= != <> ++ -- and or xor !';
     ...
     */
    private function getElement( string $buffer, string $state )
    {
     ...
     */
    public function parse( string $source_path )
    {
        ...
        if ( !$fp ) {
            throw new PhpSourceParserException( self::ERROR_FILENOTFOUND, "Source file not found:$source_path" );
        }
        $line_no = 1;
        $source = [];
        while ( !feof($fp) ) {
